...
def get_no_show_stats(bookings_df: pd.DataFrame) -> pd.DataFrame:
    """Return phone-wise no-show stats: phone, no_show_count, last_no_show_date."""
    if bookings_df.empty:
        return pd.DataFrame(columns=["phone", "no_show_count", "last_no_show_date"])
    ...
    stats.rename(columns={"phone": "phone"}, inplace=True)
    return stats
    ...
    if phone_input.strip():
        if (not phone_clean.isdigit()) or len(phone_clean) != 10:
            ...
            if user_bookings.empty:
                ...
                # (Optional) general rating section removed to avoid confusion,
                # since you wanted the popup style right after ending.
...
# --- Mode 6: Management Dashboard (Admin side) ---
    ...
    if bookings_df.empty:
        ...
        ax3.barh(loc_counts.index, loc_counts.values)
